...
| Official CCLD TransparencyAPI facility-reference source | Seven structured CSV exports plus bounded JSON/detail/report endpoints | Issue #553 authorizes `DownloadStateData` for the seven approved export IDs, `FacilityDetail/{facilityNumber}`, `Group/`, `CACounty`, and bounded `FacilityReports` list/helper calls. `FacilitySearch` enumeration and raw `REPORTPAGE` values containing `fakeout.gov` are prohibited. | No automated cadence is approved by Issue #554. Content change is determined from preserved response and normalized-content hashes, not request time alone. | Approved primary current facility-reference source family. Issue #554 implements connector/snapshot infrastructure; Issue #482 reads eligible rows from the active accepted pointer through the shared projection. Production promotion/backfill, scheduling, deployment, and hosted acceptance remain separately governed. | Six exports have 31 fixed headers and standard six-field complaint blocks; RCFE has 38 fixed headers and seven-field blocks. Facility Number is text and unique only within one accepted current snapshot. Blank/duplicate IDs, partial blocks, unknown type codes, detail sentinels, and report mismatches require quarantine. Disappearance does not imply closure/deletion. | Preserve exact response bytes before parsing; request/final URL, retrieval time, status, safe response headers, media type, disposition, byte count, SHA-256, source identity, fixed-header/taxonomy/content fingerprints, export ID, row ordinal, raw-row SHA-256, raw/normalized observations, ordered complaint blocks, warnings, quarantines, active/prior state, and rollback evidence. Cookies, credentials, private correspondence, and authentication headers are excluded. |
...
RecordsTracker's existing preload path still uses named CHHS/CDSS Community
Care Licensing Facilities program resources. Issue #490 successfully profiled
those seven program datastores as a bounded 68,527-row evaluation snapshot.
...
framing remains historical evidence. Issue #553 supersedes its forward-looking
source direction by approving TransparencyAPI as the primary current
facility-reference source. This inventory does not itself activate imports,
hosted behavior, deployment, UI changes, raw CSV commits, or profiling outputs.

Accepted TransparencyAPI snapshots are the approved primary current-reference
source family. Issue #482 reads the active accepted pointer without promoting or
modifying it. CKAN/program snapshots remain historical and controlled fallback
evidence, and ArcGIS remains a separately versioned supplementary observation.
No source overwrites or erases another source's provenance. Blank/placeholder
observations never erase populated address or telephone values; conflicting
nonblank values remain explicit. Disappearance does not mean closure or deletion,
failed candidates never partially apply, and rollback selects complete prior
accepted snapshots.
...
resource timestamps alone are insufficient. Facility Number is unique only
within one accepted current TransparencyAPI snapshot and is not a permanent
identity; it also cannot be a unique ArcGIS database key. Any later
reconciliation/backfill must retain field-level provenance and nonblank
conflicts, and raw `733` must remain unmapped unless verified source or governed
mapping evidence proves a label.
