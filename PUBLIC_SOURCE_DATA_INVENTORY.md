...
| Statewide ArcGIS-backed Community Care Licensing Facilities candidate evaluated in Issue #490 | Structured ArcGIS/open-data candidate plus catalog and export metadata | Multiple same-titled California catalog records link a statewide-looking ArcGIS item/service and several exports. Their succession or equivalence is unresolved. The current statewide service and a corresponding stable export were not jointly profiled; the legacy ZIP returned sanitized HTTP `403` after its approved redirect. | Unknown. Catalog modification dates and resource dates do not prove content freshness or justify a refresh cadence. | Currently inactive. Issue #490 verdict remains **inconclusive; retain existing program-specific sources and keep the statewide candidate inactive**. Issue #516 makes a fresh governed shadow comparison and explicit adopt/supplement/reject decision required Build Week work; it does not pre-approve activation. | Statewide schema, row count, pagination, service/export equivalence, facility-ID coverage, duplicates, fields/domains, conflicts, content change, terms, license, system-of-record, maintainer, and code `733` mapping remain unresolved. The separately profiled 15-row City of Placentia layer is not a statewide comparison source. | Before any decision or cutover, preserve and reconcile the selected catalog, item, service, layer, and stable export identities; immutable original responses; sanitized retrieval metadata; original-byte and normalized-content hashes; schema/domain fingerprints; complete pagination and equivalence evidence; publisher/maintainer/terms/license decisions; row/field coverage; conflicts; and unresolved codes. See the [Issue #490 completion report](docs/analysis/issue-490-completion-report.md) and [Build Week completion plan](docs/planning/build-week-2026-arcgis-facility-reference-completion-plan.md). |
...
Issue #516 changes the forward Build Week completion scope, not the present
source state or Issue #490 verdict. The statewide ArcGIS candidate must now be
implemented first as a governed shadow connector and compared with these CSV
snapshots and PostgreSQL rows. Only an explicit evidence-supported source
decision can authorize cutover. If adoption is supported and the complete
implementation gates pass, ArcGIS becomes the governed normal production
facility-reference source. These CSV snapshots must remain retained for
provenance, history, comparison, and controlled fallback, but are no longer the
normal active source after successful cutover. Until then, current behavior is
unchanged.
...
resource timestamps alone are insufficient. Any ArcGIS backfill must retain
field-level provenance and nonblank conflicts, and raw `733` must remain
unmapped unless verified source or governed mapping evidence proves a label.
