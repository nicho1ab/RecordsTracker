...
- The hosted seeded corpus import path is controlled and local/test-oriented.
    ...
    complaint record, open detail, view safe source traceability fields, submit
    a bounded reviewer note, submit a bounded reviewer status, and see read-
    after-write reviewer-created state. The UI uses process-local seeded test
    state and a fixture actor context; it is not production authentication,
