...
- The local hosted scaffold `/ccld/records/request` page accepts a CCLD
  ...
  recovery guidance, clearer no-match and local validated load guidance,
...
- Production-readiness items such as source-verification planning, auth provider
  ...
  concrete MVP-blocking risk.
- Controlled browser-triggered CCLD retrieval now has a first implementation
...
- `CCLD_RETRIEVAL_DEMO_MODE=mock-success` is a local-dev scaffold validation mode
  only. It uses committed fixtures to demonstrate successful retrieval/import/
