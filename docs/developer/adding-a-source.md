...
The initial deterministic extraction fixture targets report index `3`. It
extracts labeled HTML fields plus a bounded, dated investigation activity
sentence deterministically and normalizes them into the existing facility,
source document, complaint, allegation, event, and extraction audit records.
Facility address and city source elements are audited as present-but-blank in
that governed artifact; canonical allocation for those fields remains separate.
Target audit rows preserve document identity, source section, and bounded source
text while generated evidence emits only aggregate-safe status and hashes.
