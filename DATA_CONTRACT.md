...
validated hosted import artifacts. The PostgreSQL facility-reference table is a
separate source-reference projection; it does not silently merge rows into a
canonical facility entity.
...
| `facility.county` | Official facility reference `County Name` or `COUNTY`, or the same governed value in a validated canonical facility artifact. | Existing canonical `facility.county`; nullable string. Facility-reference preload holds the typed projection. | Missing, blank, whitespace-only, or unavailable is null; never an empty default. A later null does not erase an earlier non-null canonical value. | Trim surrounding whitespace; preserve the source label rather than inventing a county taxonomy. Ordering and deduplication do not apply. | Canonical artifacts retain source-document traceability; reference rows retain resource metadata and `original_row_json`. | Validated artifact/SQLite import for canonical data; facility-reference preload for reference initialization. Regenerate/reimport or rerun preload for existing rows. |
| `facility.facility_type` | CCLD `Facility Type` or CHHS `FAC_TYPE_DESC`. CHHS numeric `TYPE` is provenance only and is not a facility-type label. | Existing canonical `facility.facility_type`; nullable string. Facility-reference preload holds the typed projection. | Missing, blank, whitespace-only, or unavailable is null. A later null does not erase an earlier non-null canonical value. | Trim surrounding whitespace and preserve the governed descriptive label; do not substitute `PROGRAM_TYPE`, `CLIENT_SERVED`, or numeric `TYPE`. | Canonical artifacts retain source-document traceability; reference rows retain resource metadata and the unmodified source row. | Validated artifact/SQLite import for canonical data; facility-reference preload for reference initialization. Regenerate/reimport or rerun preload for existing rows. |
