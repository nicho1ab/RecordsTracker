...
FINDINGS_HEADING_NO_COLON_FIXTURE_URL = (
    "https://www.ccld.dss.ca.gov/transparencyapi/api/FacilityReports?facNum=157806098&inx=60"
)
...
REPORT_DATE_SPACED_COLON_FIXTURE_URL = (
    "https://www.ccld.dss.ca.gov/transparencyapi/api/FacilityReports?facNum=157806098&inx=65"
)
...
FACILITY_NUMBER_SPACED_COLON_FIXTURE_URL = (
    "https://www.ccld.dss.ca.gov/transparencyapi/api/FacilityReports?facNum=157806098&inx=70"
)
FINDING_SPACED_COLON_FIXTURE_URL = (
    "https://www.ccld.dss.ca.gov/transparencyapi/api/FacilityReports?facNum=157806098&inx=71"
)
...
VISIT_DATE_SEMICOLON_FIXTURE_URL = (
    "https://www.ccld.dss.ca.gov/transparencyapi/api/FacilityReports?facNum=157806098&inx=74"
)
...
FINDINGS_HEADING_NO_COLON_RAW_FIXTURE = Path(
    "tests/fixtures/ccld/raw/157806098_inx60_findings_heading_no_colon.html"
)
...
REPORT_DATE_SPACED_COLON_RAW_FIXTURE = Path(
    "tests/fixtures/ccld/raw/157806098_inx65_report_date_spaced_colon.html"
)
...
FACILITY_NUMBER_SPACED_COLON_RAW_FIXTURE = Path(
    "tests/fixtures/ccld/raw/157806098_inx70_facility_number_spaced_colon.html"
)
FINDING_SPACED_COLON_RAW_FIXTURE = Path(
    "tests/fixtures/ccld/raw/157806098_inx71_finding_spaced_colon.html"
)
...
VISIT_DATE_SEMICOLON_RAW_FIXTURE = Path(
    "tests/fixtures/ccld/raw/157806098_inx74_visit_date_semicolon.html"
)
...
FINDINGS_HEADING_NO_COLON_EXPECTED_FIXTURE = Path(
    "tests/fixtures/ccld/expected/157806098_inx60_findings_heading_no_colon.json"
)
...
REPORT_DATE_SPACED_COLON_EXPECTED_FIXTURE = Path(
    "tests/fixtures/ccld/expected/157806098_inx65_report_date_spaced_colon.json"
)
...
FACILITY_NUMBER_SPACED_COLON_EXPECTED_FIXTURE = Path(
    "tests/fixtures/ccld/expected/157806098_inx70_facility_number_spaced_colon.json"
)
FINDING_SPACED_COLON_EXPECTED_FIXTURE = Path(
    "tests/fixtures/ccld/expected/157806098_inx71_finding_spaced_colon.json"
)
...
VISIT_DATE_SEMICOLON_EXPECTED_FIXTURE = Path(
    "tests/fixtures/ccld/expected/157806098_inx74_visit_date_semicolon.json"
)
...
def test_ccld_facility_report_extracts_findings_heading_without_colon() -> None:
    connector = CcldFacilityReportsConnector()
    normalized = connector.normalize(_extract_findings_heading_no_colon_fixture())
    expected = json.loads(
        FINDINGS_HEADING_NO_COLON_EXPECTED_FIXTURE.read_text(encoding="utf-8")
    )

    assert _without_audit(normalized) == expected


def test_ccld_facility_report_extracts_punctuated_received_date_phrase() -> None:
    connector = CcldFacilityReportsConnector()
    normalized = connector.normalize(_extract_received_date_punctuation_fixture())
    expected = json.loads(
...
def test_ccld_facility_report_extracts_dashed_findings_heading() -> None:
    connector = CcldFacilityReportsConnector()
...
def test_ccld_facility_report_extracts_spaced_colon_report_date_label() -> None:
    connector = CcldFacilityReportsConnector()
    normalized = connector.normalize(_extract_report_date_spaced_colon_fixture())
    expected = json.loads(
        REPORT_DATE_SPACED_COLON_EXPECTED_FIXTURE.read_text(encoding="utf-8")
    )

    assert _without_audit(normalized) == expected


def test_ccld_facility_report_extracts_spaced_colon_date_signed_label() -> None:
    connector = CcldFacilityReportsConnector()
...
def test_ccld_facility_report_extracts_spaced_colon_complaint_control_label() -> None:
    connector = CcldFacilityReportsConnector()
...
def test_ccld_facility_report_extracts_spaced_colon_facility_number_label() -> None:
    connector = CcldFacilityReportsConnector()
    normalized = connector.normalize(_extract_facility_number_spaced_colon_fixture())
    expected = json.loads(
        FACILITY_NUMBER_SPACED_COLON_EXPECTED_FIXTURE.read_text(encoding="utf-8")
    )

    assert _without_audit(normalized) == expected


def test_ccld_facility_report_extracts_split_spaced_colon_finding_label() -> None:
    connector = CcldFacilityReportsConnector()
    normalized = connector.normalize(_extract_finding_spaced_colon_fixture())
    expected = json.loads(
        FINDING_SPACED_COLON_EXPECTED_FIXTURE.read_text(encoding="utf-8")
    )

    assert _without_audit(normalized) == expected


def test_ccld_facility_report_extracts_semicolon_report_date_label() -> None:
    connector = CcldFacilityReportsConnector()
...
def test_ccld_facility_report_extracts_semicolon_visit_date_label() -> None:
    connector = CcldFacilityReportsConnector()
    normalized = connector.normalize(_extract_visit_date_semicolon_fixture())
    expected = json.loads(
        VISIT_DATE_SEMICOLON_EXPECTED_FIXTURE.read_text(encoding="utf-8")
    )

    assert _without_audit(normalized) == expected


def test_ccld_facility_report_extracts_semicolon_complaint_control_label() -> None:
    connector = CcldFacilityReportsConnector()
...
def test_ccld_facility_report_extracts_semicolon_facility_number_label() -> None:
    connector = CcldFacilityReportsConnector()
    normalized = connector.normalize(_extract_facility_number_semicolon_fixture())
    expected = json.loads(
        FACILITY_NUMBER_SEMICOLON_EXPECTED_FIXTURE.read_text(encoding="utf-8")
    )
...
def _extract_findings_heading_no_colon_fixture() -> dict[str, object]:
    raw_content = FINDINGS_HEADING_NO_COLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=FINDINGS_HEADING_NO_COLON_FIXTURE_URL,
        raw_path=FINDINGS_HEADING_NO_COLON_RAW_FIXTURE,
        raw_sha256=sha256_bytes(raw_content),
        retrieved_at=FINDINGS_HEADING_NO_COLON_RETRIEVED_AT,
        content_type="text/html",
    )
    return CcldFacilityReportsConnector().extract(document)


def _extract_received_date_punctuation_fixture() -> dict[str, object]:
    raw_content = RECEIVED_DATE_PUNCTUATION_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=RECEIVED_DATE_PUNCTUATION_FIXTURE_URL,
...
def _extract_findings_heading_dash_fixture() -> dict[str, object]:
    raw_content = FINDINGS_HEADING_DASH_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=FINDINGS_HEADING_DASH_FIXTURE_URL,
...
def _extract_report_date_spaced_colon_fixture() -> dict[str, object]:
    raw_content = REPORT_DATE_SPACED_COLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=REPORT_DATE_SPACED_COLON_FIXTURE_URL,
        raw_path=REPORT_DATE_SPACED_COLON_RAW_FIXTURE,
        raw_sha256=sha256_bytes(raw_content),
        retrieved_at=REPORT_DATE_SPACED_COLON_RETRIEVED_AT,
        content_type="text/html",
    )
    return CcldFacilityReportsConnector().extract(document)


def _extract_date_signed_spaced_colon_fixture() -> dict[str, object]:
    raw_content = DATE_SIGNED_SPACED_COLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=DATE_SIGNED_SPACED_COLON_FIXTURE_URL,
...
def _extract_complaint_control_spaced_colon_fixture() -> dict[str, object]:
    raw_content = COMPLAINT_CONTROL_SPACED_COLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=COMPLAINT_CONTROL_SPACED_COLON_FIXTURE_URL,
...
def _extract_facility_number_spaced_colon_fixture() -> dict[str, object]:
    raw_content = FACILITY_NUMBER_SPACED_COLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=FACILITY_NUMBER_SPACED_COLON_FIXTURE_URL,
        raw_path=FACILITY_NUMBER_SPACED_COLON_RAW_FIXTURE,
        raw_sha256=sha256_bytes(raw_content),
        retrieved_at=FACILITY_NUMBER_SPACED_COLON_RETRIEVED_AT,
        content_type="text/html",
    )
    return CcldFacilityReportsConnector().extract(document)


def _extract_finding_spaced_colon_fixture() -> dict[str, object]:
    raw_content = FINDING_SPACED_COLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=FINDING_SPACED_COLON_FIXTURE_URL,
        raw_path=FINDING_SPACED_COLON_RAW_FIXTURE,
        raw_sha256=sha256_bytes(raw_content),
        retrieved_at=FINDING_SPACED_COLON_RETRIEVED_AT,
        content_type="text/html",
    )
    return CcldFacilityReportsConnector().extract(document)


def _extract_report_date_semicolon_fixture() -> dict[str, object]:
    raw_content = REPORT_DATE_SEMICOLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=REPORT_DATE_SEMICOLON_FIXTURE_URL,
...
def _extract_visit_date_semicolon_fixture() -> dict[str, object]:
    raw_content = VISIT_DATE_SEMICOLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=VISIT_DATE_SEMICOLON_FIXTURE_URL,
        raw_path=VISIT_DATE_SEMICOLON_RAW_FIXTURE,
        raw_sha256=sha256_bytes(raw_content),
        retrieved_at=VISIT_DATE_SEMICOLON_RETRIEVED_AT,
        content_type="text/html",
    )
    return CcldFacilityReportsConnector().extract(document)


def _extract_complaint_control_semicolon_fixture() -> dict[str, object]:
    raw_content = COMPLAINT_CONTROL_SEMICOLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=COMPLAINT_CONTROL_SEMICOLON_FIXTURE_URL,
...
def _extract_facility_number_semicolon_fixture() -> dict[str, object]:
    raw_content = FACILITY_NUMBER_SEMICOLON_RAW_FIXTURE.read_bytes()
    document = SourceDocument(
        source_url=FACILITY_NUMBER_SEMICOLON_FIXTURE_URL,
        raw_path=FACILITY_NUMBER_SEMICOLON_RAW_FIXTURE,
        raw_sha256=sha256_bytes(raw_content),
        retrieved_at=FACILITY_NUMBER_SEMICOLON_RETRIEVED_AT,
        content_type="text/html",
    )
    return CcldFacilityReportsConnector().extract(document)
